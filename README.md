...
Published images (multi-platform `linux/amd64` + `linux/arm64`), built from this repo's `Dockerfile` with the **Cursor CLI (`cursor-agent`) preinstalled**:

- Docker Hub: [`decolua/9router`](https://hub.docker.com/r/decolua/9router)
- GHCR: [`ghcr.io/decolua/9router`](https://github.com/decolua/9router/pkgs/container/9router)

See [DOCKER.md](DOCKER.md) for the full guide (volumes, Cursor CLI usage, env vars).
...
**Quick start (build locally, includes Cursor CLI):**
...
docker run -d --name 9router -p 20128:20128 \
  -v "$HOME/.9router:/app/data" -e DATA_DIR=/app/data 9router:latest
```

**Volumes to mount:**

- `/app/data` — 9Router DB/config/certs (mount `$HOME/.9router`). Recommended.
- `/home/node/.cursor` — optional; persists Cursor CLI login/state.

**Cursor CLI (preinstalled in the image):**

```bash
docker exec -it 9router cursor-agent --version
docker exec -it 9router cursor-agent login   # persists with /home/node/.cursor mounted
```

> The Cursor CLI requires glibc, so this image uses Debian slim (glibc).
...
- Both `${DATA_DIR}` and `~/.9router` resolve to the same location in a Docker container because `DATA_DIR=/app/data` is set in the image.
